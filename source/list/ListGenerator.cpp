...
{
    clear();
    std::vector<dir_discHdr>().swap(m_buffer);
}
...
void ListGenerator::FastLoadDB(const std::string& dbPath)
{
    FILE* cache_file = fopen(dbPath.c_str(), "rb");
    if(cache_file != NULL)
    {
        fseek(cache_file, 0, SEEK_END);
        long total_bytes = ftell(cache_file);
        rewind(cache_file);

        int count = total_bytes / sizeof(dir_discHdr);
        
        m_buffer.resize(count); 

        if (count > 0) {
            fread(&m_buffer[0], sizeof(dir_discHdr), count, cache_file);
        }
...
void ListGenerator::FastLoadMultiDB(const std::vector<std::string>& dbPaths, std::vector<dir_discHdr>& targetList)
{
    gprintf("ListGenerator::FastLoadMultiDB() - Starting load of %d databases\n", dbPaths.size());
    mainMenu.m_status.Add("Loading library...");
    
    // 1. Pre-calculate total for progress tracking
    long total_bytes = 0;
    for(size_t idx = 0; idx < dbPaths.size(); idx++)
    {
        FILE* f = fopen(dbPaths[idx].c_str(), "rb");
        if(f != NULL) {
            fseek(f, 0, SEEK_END);
            total_bytes += ftell(f);
            fclose(f);
    ...
    int total_count = total_bytes / sizeof(dir_discHdr);
    targetList.reserve(targetList.size() + total_count + 1000);
    ...
    for(size_t idx = 0; idx < dbPaths.size(); idx++)
    {
        const char* fileName = strrchr(dbPaths[idx].c_str(), '/');
        
        mainMenu.m_status.Add(fmt("Now loading database: %s", fileName ? fileName + 1 : dbPaths[idx].c_str()));
        gprintf("ListGenerator::FastLoadMultiDB() - Now loading database: %s\n", dbPaths[idx].c_str());
        ...
        if(f != NULL) 
        {
            ...
            int file_count = file_bytes / sizeof(dir_discHdr);
            if (file_count > 0) 
            {
                size_t current_size = targetList.size();
                targetList.resize(current_size + file_count);
                
                fread(&targetList[current_size], sizeof(dir_discHdr), file_count, f);
                
                gamesLoaded += file_count;
                
                if ((gamesLoaded / 25) > (lastCheckpoint / 25)) {
                    mainMenu.m_status.Add(fmt("Loaded %d / %d games", gamesLoaded, total_count));
                    lastCheckpoint = gamesLoaded;
                }
            }
            fclose(f);
        }
		...
		{
			mainMenu.m_status.Add(fmt("Failed to open database: %s", fileName ? fileName + 1 : dbPaths[idx].c_str()));
        	gprintf("ListGenerator::FastLoadMultiDB() - Failed to open: %s\n", dbPaths[idx].c_str());
		}
    }
    
    mainMenu.m_status.Add("Library loaded!");
    gprintf("ListGenerator::FastLoadMultiDB() - Finished! Total: %d games\n", targetList.size());
}
...
void ListGenerator::ParseScummvmINI(Config &ini, const char *Device, const char *datadir, const char *platform, const string& DBName, bool UpdateCache)
{
    ...
    while(1)
    {
        ...
        ListElement.settings[0] = m_cacheList.Magic; 
        ListElement.casecolor = m_cacheList.Color;
        ListElement.type = TYPE_PLUGIN;
        m_cacheList.push_back(ListElement);
        GameDomain = ini.nextDomain().c_str();
...
void ListGenerator::CreateRomList(const char *platform, const string& romsDir, const vector<string>& FileTypes, const string& DBName, bool UpdateCache)
{
    ...
    GetFiles(romsDir.c_str(), FileTypes, Add_Plugin_Game, false, 30);
...
{
	mainMenu.m_status.Add("Starting CreateList...");
