...
int https_connect(char *host, u16 port)
{
...
void downloadfile(const char *url, struct download *buffer)
{
    ...
    if (getProxyAddress() && getProxyPort() > 0)
        httpinfo.sock = https_connect(getProxyAddress(), getProxyPort());
    else
        httpinfo.sock = https_connect(host, httpinfo.use_https ? 443 : 80);
